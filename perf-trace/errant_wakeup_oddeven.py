...
# This script works for sd_llc=4 but still keep WAKEUP_SCOPE_SIZE=8, bcz
# this script will remove odd or even numbered threads based on common_cpu.
WAKEUP_SCOPE_SIZE = 8 # odd/even thread
LATENCY_THRESHOLD_US = (10**3)*10 # 10ms
VERBOSE = False
OFFLINE_CPUS = range(64, 80)
                ...
                if (confidence[target_cpu] > NO_CONFIDENCE):
                        ...
                        if (runqlen[target_cpu] > 1):
                                ...
                                for i in range(first_cpu, last_cpu):
                                    if (is_odd_thread == 1 and i%2 == 0):
                                        continue
                                    if (is_odd_thread == 0 and i%2 ==1):
                                        continue
